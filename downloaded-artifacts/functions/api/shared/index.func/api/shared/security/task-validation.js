...
exports.createTaskSecurely = createTaskSecurely;
exports.updateTaskSecurely = updateTaskSecurely;
exports.validateParentTaskOwnership = validateParentTaskOwnership;
 ...
 */
async function validateParentTaskOwnership(context, userId, parentTaskId) {
  if (!parentTaskId) {
    return {
      valid: true
    }; // No parent task is valid
  }
  ...
  try {
    ...
    const {
      data: parentTask,
      error
    } = await context.supabase.from('tasks').select('id, user_id').eq('id', parentTaskId).single();
    if (error) {
      return {
        valid: false,
 ...
 */
async function validateTaskCreation(context, taskInput) {
 ...
 */
async function validateTaskUpdate(context, taskId, updates) {
  ...
  if (updates.parent_task_id !== undefined || updates.user_id !== undefined) {
    ...
    const {
      data: currentTask,
      error
    } = await context.supabase.from('tasks').select('user_id').eq('id', taskId).single();
 ...
 */
async function createTaskSecurely(context, taskInput) {
  ...
  const {
    data,
    error
  } = await context.supabase.from('tasks').insert(taskInput).select().single();
  if (error) {
 ...
 */
async function updateTaskSecurely(context, taskId, updates) {
  ...
  const {
    data,
    error
  } = await context.supabase.from('tasks').update(updates).eq('id', taskId).select().single();
  if (error) {
